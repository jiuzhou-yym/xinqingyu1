      ...
      <div style={{ 
        ...
        {wordCloudData.length > 0 ? (
          wordCloudData.map((word, index) => {
            // 计算基础大小（与频率成正比）
            const baseSize = word.value / 1.5 + 10; // 增大字体大小
            
            // 使用有序分布避免重叠，按同心圆和角度均匀分布
            const rings = 3; // 分成3个同心圆环
            const ringIndex = Math.floor(index / (wordCloudData.length / rings)); // 确定词放在哪个环
            const ringRadius = 20 + ringIndex * 15; // 每个环的半径不同
            
            // 在环内均匀分布角度
            const angleStep = (2 * Math.PI) / Math.ceil(wordCloudData.length / rings);
            const angle = (index % Math.ceil(wordCloudData.length / rings)) * angleStep + Math.random() * 0.3; // 添加小随机变化
            
            // 计算位置，确保词语分布均匀
            const left = 50 + Math.cos(angle) * ringRadius + '%';
            const top = 50 + Math.sin(angle) * ringRadius + '%';
            
            // 轻微随机旋转增加自然感，但限制范围减少重叠
            ...
            return (
              <span
                ...
                style={{
                   ...
                   padding: `${baseSize / 4}px ${baseSize / 2}px`, // 增加内边距，减少重叠
                   backgroundColor: 'transparent',
                   color: getRandomColor(),
                   borderRadius: '4px',
                   opacity: 0.9 + (word.value / 200),
                   zIndex: Math.round(word.value * 2), // 增加z-index差异，确保重要词语在上层
                   transition: 'all 0.3s ease',
                   fontWeight: '500',
                   textShadow: '1px 1px 2px rgba(0,0,0,0.15)'
                 }}
                onMouseEnter={(e) => {
                  e.target.style.transform = `translate(-50%, -50%) rotate(${rotation}deg) scale(1.1)`;
                  e.target.style.zIndex = '100';
                }}
                onMouseLeave={(e) => {
                  e.target.style.transform = `translate(-50%, -50%) rotate(${rotation}deg) scale(1)`;
                  e.target.style.zIndex = Math.round(word.value);
                }}
              >
                {word.text}
              </span>
